...
test_versions=("2022.1.0a13" "2021.1.28f1" "2021.2.2f1" "2020.1.17f1" "2020.2.7f1" "2020.3.22f1" "2019.1.14f1" "2019.2.21f1" "2019.3.15f1" "2019.4.32f1" "2018.4.36f1" "2018.3.14f1" "2018.2.21f1" "2018.1.9f2" "2017.4.40f1")
...
do
    ...
    do
        ...
        *a*)
        ...
        ./target/debug/uvm download-manifest -o "${output_dir}" -n "${version}_manifest.ini" -p "${platform}" -f -v "${version}"
        if [ "$platform" = "${local_platform}" ]; then
            module_src="${hub_isntall_dir}/${version}/modules.json"
            ...
            if ! grep -q -F "    ${module_def}" "${modfile}"; then
                ...
                sed -i "s/\/\/MODULE_JSON/\/\/MODULE_JSON\n    ${module_def}/i" $modfile
            fi
            ...
            if ! grep -q -F "    ${manifest_def}" "${modfile}"; then
                echo "add ${version} module fixum to test setup"
                sed -i "s/\/\/MANIFEST_INI/\/\/MANIFEST_INI\n    ${manifest_def}/i" $modfile
            fi
