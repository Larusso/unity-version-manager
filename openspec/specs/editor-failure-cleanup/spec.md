# Spec: Editor Failure Cleanup

## Purpose

When a Unity Editor installation fails during a fresh install, the system should clean up the partially created installation directory to leave no partial state. This prevents orphaned directories from appearing in `uvm list` and ensures the system remains in a consistent state.
